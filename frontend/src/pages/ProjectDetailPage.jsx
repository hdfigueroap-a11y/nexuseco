...
// Vista detallada de un proyecto con tabs: Indicadores · Alertas · Evidencias · Historial
...
const TABS = ['Indicadores', 'Alertas', 'Evidencias', 'Historial'];
...
export default function ProjectDetailPage() {
  ...
  if (loading) return <div className="flex justify-center py-16"><div className="animate-spin rounded-full h-10 w-10 border-b-2 border-eco-600" /></div>;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card">
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
          ...
          <div className="flex gap-4 shrink-0">
            <MetricBox
              value={`${project.current_ivi ?? '—'}%`}
              label="IVI"
              critical={!iviOk}
            />
            <MetricBox
              value={`${project.compliance_pct ?? '—'}%`}
              label="Cumplimiento"
        ...
        {project.area_km2 && (
          <div className="mt-4 pt-4 border-t border-gray-100 text-sm text-gray-500">
            📍 Área cubierta: <strong>{project.area_km2} km²</strong>
            <span className="ml-4">
              N:{project.north} | S:{project.south} | E:{project.east} | O:{project.west}
            </span>
          </div>
        )}
      ...
      <div>
        ...
        {activeTab === 'Historial' && user?.role !== 'auditor' && (
          <div className="card text-center text-gray-400 py-10">
            🔍 El historial de cambios es visible únicamente para el Auditor.
          </div>
        )}
      </div>
    </div>
